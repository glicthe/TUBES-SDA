...
static NodeType determineChildType(int childDepth, boolean* isRestPlaced, boolean* isMerchantPlaced, char* nameBuffer) {
    ...
    if (childDepth == 4) {
        snprintf(nameBuffer, 50, "TERRITORY GUARDIAN");
        return NODE_SECONDARY_BOSS;
    }
    if (childDepth == 6) {
        snprintf(nameBuffer, 50, "THRONE OF THE FINAL RULER");
        return NODE_BIG_BOSS;
    }

    while (!typeFinalized && attemptCount < 20) {
        attemptCount++;
        typeRoll = generateRandomRange(0, 99);
        
        if (childDepth == 1) {
            snprintf(nameBuffer, 50, "Initial Outpost");
            typeFinalized = TRUE;
        } else if (childDepth == 2 || childDepth == 3 || childDepth == 5) {
            if (typeRoll < PROB_ENEMY_COMMON) {
                snprintf(nameBuffer, 50, "Common Obstacle");
                typeFinalized = TRUE;
            } else if (typeRoll < PROB_REST_NODE) {
                if (!(*isRestPlaced)) {
                    type = NODE_REST;
                    snprintf(nameBuffer, 50, "Rest Site");
                    *isRestPlaced = TRUE;
                    typeFinalized = TRUE;
                }
            } else {
                if (!(*isMerchantPlaced)) {
                    type = NODE_MERCHANT;
                    snprintf(nameBuffer, 50, "Merchant");
                    *isMerchantPlaced = TRUE;
                    typeFinalized = TRUE;
                }
            }
        } else {
            snprintf(nameBuffer, 50, "Emergency Node");
            typeFinalized = TRUE;
        }
    }
    if (!typeFinalized) {
        snprintf(nameBuffer, 50, "Emergency Path");
    }
    return type;
}
